...
fun EnterpriseEditScreen(
    ...
    val name = remember { mutableStateOf(enterprise.name) }
    val locationName = remember { mutableStateOf(enterprise.locationName ?: "") }
    val latitude = remember { mutableStateOf(enterprise.latitude?.toString().orEmpty()) }
    val longitude = remember { mutableStateOf(enterprise.latitude?.toString().orEmpty()) }
    val urlImage = remember { mutableStateOf<String?>(enterprise.urlImage) }
                        ...
                        if (errorsForm.any()) {
                            ...
                            listener.update(
                                    Enterprise(
                                        id = enterprise.id,
                                        name = name.value,
                                        locationName = if (locationName.value.trim()
                                                .isEmpty()
                                        ) null else locationName.value,
                                        latitude = if (latitude.value.trim()
                                                .isEmpty()
                                        ) null else latitude.value.toDoubleOrNull(),
                                        longitude = if (longitude.value.trim()
                                                .isEmpty()
                                        ) null else longitude.value.toDoubleOrNull(),
                                        urlImage.value
                                    )
                                )
...
private fun EnterpriseEditScreenEditScreenPreview() {
    ...
    EnterpriseEditScreen(
        enterprise = Enterprise(1, "Totyota", null, null, null, null),
        listener = ControllerProvider().enterpriseController,
        uiProvider = uiProvider
    )
}
